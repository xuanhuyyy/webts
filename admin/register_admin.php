...
if(isset($_POST['submit'])){
   ...
   if($select_admin->rowCount() > 0){
      echo '<script language="javascript">alert("Tên đăng nhập đã tồn tại!"); window.location="register_admin.php";</script>';
   }else{
      if($pass != $cpass){
         echo '<script language="javascript">alert("Xác nhận mật khẩu không khớp!"); window.location="register_admin.php";</script>';
      }else{
         $insert_admin = $conn->prepare("INSERT INTO `admins`(name, password) VALUES(?,?)");
         $insert_admin->execute([$name, $cpass]);
         echo '<script language="javascript">alert("Đăng ký thành công!"); window.location="dashboard.php";</script>';
      }
   }
